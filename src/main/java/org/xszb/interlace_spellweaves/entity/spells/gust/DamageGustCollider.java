...
public class DamageGustCollider extends AbstractConeProjectile {
    protected float damage;

    public DamageGustCollider(Level level, LivingEntity owner) {
        this(RegistryEntity.GUST_COLLIDER.get(), level);
        this.setOwner(owner);
        this.setRot(owner.getYRot(), owner.getXRot());
    }

    public DamageGustCollider(EntityType<DamageGustCollider> gustColliderEntityType, Level level) {
        ...
        if (entity != null && resultEntity instanceof LivingEntity target && target.distanceToSqr(entity) < range * range)
            if (!DamageSources.isFriendlyFireBetween(entity, target)) {
                ...
                if (entity instanceof NamelessWizardsEntity ent){
                    ent.setHealthAttack(2.5f,target);
                }

                DamageSources.applyDamage(target, getDamage(), RegistrySpell.GUST_PRO.get().getDamageSource(this, getOwner()));
            }
