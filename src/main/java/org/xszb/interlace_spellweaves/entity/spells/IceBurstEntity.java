...
public class IceBurstEntity extends AoeEntity {
    ...
    public IceBurstEntity(Level level, LivingEntity owner, float damage, float radius) {
        this(RegistryEntity.ICE_BURST.get(), level);
        setOwner(owner);
    ...
    public void tick() {
        if (tickCount == waitTime) {
            this.playSound(SoundRegistry.FROSTWAVE_PREPARE.get(), 1, Utils.random.nextIntBetweenInclusive(8, 12) * .1f);
            if (!level().isClientSide) {
                var center = this.getBoundingBox().getCenter();
                MagicManager.spawnParticles(level(), ParticleHelper.SNOWFLAKE, center.x, center.y, center.z, 25, 0, 0, 0, .18, false);
                MagicManager.spawnParticles(level(), new BlastwaveParticleOptions(RegistrySpell.CHARGE_RAY_OF_FROST_SPELL.get().getSchoolType().getTargetingColor(), this.getRadius() * .9f), center.x, center.y, center.z, 1, 0, 0, 0, 0, true);
        ...
        if (level().isClientSide && tickCount < waitTime / 2) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                level().addParticle(ParticleHelper.SNOWFLAKE, vec3.x, vec3.y - .5, vec3.z, motion.x, motion.y, motion.z);
            }
        }
    }
